...
namespace ACMWeb.Controllers
{
    public class PIMappingsController : Controller
    {
        ...
        public void PIScore(int PIID) {
            var pi = db.PIMappings.Where(xx => xx.PIID==PIID).Select(x => new PIResultsVM
            {
                PI = x.PI.PILetter,
                desc = x.PI.PIDesc,
                PIno = x.PI.PINo,
                semester = x.Semester.ShortName,
                SO = x.PI.LO.LearningOutcome,
                CourseId = x.CourseID,
